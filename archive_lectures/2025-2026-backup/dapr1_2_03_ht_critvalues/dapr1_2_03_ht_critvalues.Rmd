...
+ The t-statistic is the difference between the sample and hypothesised mean, divided by the variation in sample means due to random sampling.
...
- The test statistic for testing a mean is the __t-statistic__ or __t-score__. In the example above, $\mu_0 = 20$:
...
plt.t <- ggplot(tibble(t = tscores.sampl.dist)) + 
    geom_histogram(aes(x = t, y = ..density..), 
                   ...
                   color = 'gray50', fill = 'gray90') +
    stat_function(aes(x = t), fun = dnorm, color = 'black', alpha = 1,
                  size = 1, linetype = 2, n = 501) +
    stat_function(aes(x = t), fun = \(x) dt(x, df = n-1), alpha = 1,
                  color = 'blue', size = 1, n = 501, linetype = 1) +
    labs(x = 't-statistic', y = 'Density',
         caption = 'Black dashed line: Standard Normal\nBlue solid line: t(n-1)') +
...
.pull-left[
- Suppose you have collected data on one sample, with sample size $n = 4$. The sample data are:
$$(32, 36, 26, 28)$$
...
ggplot(plt, aes(x, y)) +
    ...
    geom_vline(xintercept = tobs, color = 'blue', size = 1, linetype = 2) +
    annotate('text', tobs + 0.3, 0.03, 
             label = expression(t[obs]),
             color = 'blue', size = 7, adj = 0) +
    labs(x = 't-statistic', y = 'Density',
...
- In doing so, we compared the _blue_ area, corresponding to the p-value, against the _red_ area, corresponding to the $\alpha = 0.05$ significance level.
...
ggplot(plt) +
    ...
    geom_area(data = plt.pval,
              aes(x = x, y = y), fill = 'blue', alpha = 0.75) +
...
.pull-left[
- Rather than comparing the area of $\alpha$ (0.05, in red) to the area of the p-value (0.018, in blue), we can compare the corresponding t-statistics along the x-axis.

- The p-value is computed using the observed t-statistic, $t_{obs}$ = `r tobs |> round(2)`.
...
- We reject $H_0$ when either $t_{obs} \leq -t^*$ or $t_{obs} \geq +t^*$.
]
...
- Suppose now that the collected sample, with sample size $n = 4$, was:
$$(18, 21, 19, 23)$$
...
- Is the observed $t_{obs} =$ `r round(tobs, 2)` smaller than or equal to the lower critical value? _No!_
- Is the observed $t_{obs} =$ `r round(tobs, 2)` greater than or equal to the upper critical value? _No!_
...
- The observed t-statistic is $t_{obs}$ = `r round(tobs, 2)`.
...
> As the observed t-statistic lies beyond the critical values, the sample results provide significant evidence against the null hypothesis and in favour of the alternative one that the average body temperature differs from 37 °C; $t(49) = -3.14, p < .05$, two-sided.
...
mosaic::xqt(0.05, df = 3, xlim = c(-7, 7), 
            return = 'plot') +
    scale_fill_manual(
        values = c('red', 'gray90')
    ) +
...
.pull-left[
- $t_{obs}$ = A will lead to a p-value < .05

- $t_{obs}$ = B will lead to a p-value > .05

- $t_{obs}$ = C will lead to a p-value > .05
]
...
mosaic::xqt(0.05, df = 3, xlim = c(-7, 7), 
    ...
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu < mu[0])) +
    geom_vline(xintercept = c(-5.2, -1.8, 1.5), colour = 'blue',
               size = 1, linetype = 2) +
    annotate('label', x = c(-5.2, -1.8, 1.5), y = c(0.1, 0.1, 0.1), 
             label = c('A', 'B', 'C'), size = 6, colour = 'blue') +
    geom_vline(xintercept = tcrit, col = 'red', size = 1) +
    annotate('label', x = -5, y = 0.2, colour = 'red', size = 6,
             label = expression(alpha == 0.05)) +
    annotate('text', x = tcrit - 0.2, y = -0.02, hjust = 1,
...
mosaic::xqt(0.95, df = 3, xlim = c(-7, 7), 
            return = 'plot') +
    scale_fill_manual(
        values = c('gray90', 'red')
    ) +
...
mosaic::xqt(0.95, df = 3, xlim = c(-7, 7), 
    ...
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu > mu[0])) +
    geom_vline(xintercept = c(-1.5, 1.8, 5.2), colour = 'blue',
               size = 1, linetype = 2) +
...
mosaic::xqt(c(0.025, 0.975), df = 3, xlim = c(-7, 7), 
            return = 'plot') +
    scale_fill_manual(
...
- $t_{obs}$ = D will lead to a p-value < .05
]
...
mosaic::xqt(c(0.025, 0.975), df = 3, xlim = c(-7, 7), 
    ...
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu != mu[0])) +
    geom_vline(xintercept = c(-5.3, -2, 1.3, 3.7), colour = 'blue',
               size = 1, linetype = 2) +
    annotate('label', x = c(-5.3, -2, 1.3, 3.7), y = c(0.1, 0.1, 0.1, 0.1), 
             label = c('A', 'B', 'C', 'D'), size = 6, colour = 'blue') +
    geom_vline(xintercept = tcrit, col = 'red', size = 1) +
    annotate('label', x = -5, y = 0.2, colour = 'red', size = 6,
             label = expression(frac(alpha,2) == 0.025)) +
    annotate('label', x = 5, y = 0.2, colour = 'red', size = 6,
