...
```
We can see that the data seem to be shaped like a normal distribution, but we can also overlay a normal curve to check how well the data follow the shape of the curve. We can do this using the `dnorm` function, which produces the probability density of a normal distribution with a given mean and standard deviation. Similar to the `rnorm` function, the default `mean` and `sd` values are 0 and 1, respectively. Note that if we want to overlay a normal curve, we have to specify that the $y$-axis of the histogram should show density rather than raw count data. 

```{r}
ggplot(dat, aes(Time)) + geom_histogram(aes(y=after_stat(density)), binwidth = 2, colour = 'black') + 
  stat_function(fun = dnorm, args = list(mean = mean(dat$Time), sd = sd(dat$Time)))
```

Now let's check the `Score` variable, with the normal curve. This time, let's add a red dashed line to show the mean.

```{r}
p <- ggplot(dat, aes(Score)) + geom_histogram(aes(y=after_stat(density)), binwidth = 2, colour = 'black') + 
...
Here, I'll create a dataset that summarises our results that I can use to create a table for our write-up:
```{r}
...
The probability of students getting each letter mark can be found in Table \@ref(tab:probTab).
...
95% of the students were able to complete the exam in `r round(qnorm(.95, mean=mean(dat$Time), sd=sd(dat$Time)), 2)` minutes.
