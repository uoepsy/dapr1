...
head(temp_data)      # top six rows of the data
...
temp_data %>%
    drop_na()
# drops all rows with NAs in any of the columns
# even columns which we don't use

temp_data %>%
    drop_na(BodyTemp)
# drops rows where there are NAs in the column of interest

# there's no NA in BodyTemp variable, so data won't change in this example
# if there were any NAs in the variable of interest (BodyTemp in this case), 
# this would be the correct way to remove the rows where there are NAs only in 
# the variable of interest

temp_data <- temp_data %>%
    drop_na(BodyTemp)

# there's two variables in the data, but we are only analysing BodyTemp, so let's
# subset the data and only select that column
