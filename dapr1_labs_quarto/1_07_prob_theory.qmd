...
__This week you will only focus on task B1.__ 
...
[^top3-filter]: Hint: use `sort()` to find top 3 movie genres, and `filter()` to select the specific 3 genres. 
                Example: For the `starwars` dataset, we can filter to include only Humans and Droids using the following code:
...
- Create a variable called "Rating" where the Audience Score variable is recoded so that those scoring less than or equal to 50 are coded as "Bad" and those scoring over 50 are "Good"[^func-if-else].
...
[^func-factor]: Hint: use the `factor()` function.
                Example: If I wanted to give better labes for my size variable, I could use the following code. Since the names are fine as is for species, I will just specify `factor()`.

```{r}
starwars2$size <- factor(
                  starwars2$size,
                  levels = c("s", "t"),
                  labels = c("short", "tall"))
...
[^func-addmargins]: Hint: Make sure to load the `ggmosaic()` package so that you can specify `geom_mosaic()` when building your plot. To add a title, as well as x- and y-axis titles, specify `labs(title = , x = , y = )`
...
m_plot <- ggplot(starwars2) +
    ...
    labs(title='Starwars Mosaic Plot Example Title', x = "Size", y = "Species")
m_plot

#why are there NAs in above??
```
    
- Write up a summary of what you have reported above, using proper rounding to 2 decimal places and avoiding any reference to R code or functions.
...
Because we want to include severs A and B, but **not** C, we can use the `!=` (or does not equal) operator[^help-operators]. 

[^help-operators]: Hint: Some common operators include: 
...
```{r}
#| label: fig-mos
#| tbl-cap: Association between Servers and Tips
library(ggmosaic)
