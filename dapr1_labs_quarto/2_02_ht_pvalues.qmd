...
> At the 5% significance level and using the p-value method, test whether the mean graduation rate for female students at colleges and universities in the United States is significantly different from a rate of 50 percent.  
...
[^hypotheses]: Hint: Identify the hypothesised value of the mean, $\mu_0$, and replace that value in the following equations:
    ...
    You can write mathematical equations in the text part of an Rmd file, i.e. not inside a code chunk. All you need to do is wrap them with dollar signs to tell R when an equation starts and ends. Any text written within curly braces after an underscore is rendered as a subscript. The following lines define the null and alternative hypothesis:
