...
[^mean-se]: Hint: You can calculate the mean and standard error as: 
    ...
    * Standard error: The standard error of the mean, denoted $SE$, is the standard deviation of its sampling distribution. The standard error can be calculated as $SE = \sigma / \sqrt{n}$ but, we also need to estimate the population standard deviation $\sigma$ with the sample standard deviation $s$. The sample estimate of the standard error is:
$$
SE = \frac{s}{\sqrt{n}}
$$
...
- Visualise the average audience scores and SEs vary between the Lead Studios[^assoc-viz].
