...
Your group must submit one PDF file for formative report A by 12 noon on Friday the 1st of December 2023 (next week).
...
- Consider the `IQ1`-`IQ50` variables, are they discrete or continuous?^[Hint: Technically, the values you see in the data are discrete, but in Psychology IQ is assumed to be continuous and it's only discrete due to a limitation of our measuring ability. In other words, if we could measure that accurately, we could get an IQ of 126.4374325677...  
...
- Currently the data are in wide format, but we want a single column of IQ scores. This is so that we can give that column to `ggplot` to plot the IQ scores of all raters across all movies. Pivot the data so that it is in long format.[^func-pivot]
