...
```{r}
movies2$snack_25 <- dbinom(x = 25, size = 50, prob = 0.48)
```
...
```{r}
movies2$snack_cumul_prob <- pbinom(q = 25, size = 50, prob = 0.48, lower.tail=FALSE)
```
...
As noted above, the new movie theatre is still in the planning phase, so planners are reviewing whether they should increase their car park space, or add a bus stop on the theatre grounds. A recent survey suggested that 70% of movie viewers travel to the cinema via private transport. 
...
- To calculate the parameter $P$ of the binomial from the sample data, we could run the follwoing code: 

```{r}
p_coffee <- mean(tips$HadCoffee) / 2
p_coffee 
```
...
```{r}
tips$coffee_est <- rbinom(99, 2, 0.83)
tips$coffee_est
```
...
```{r}
tips$coffee_cumul_prob <- pbinom(1, 2, 0.83, lower.tail = FALSE)
tips$coffee_cumul_prob 
```

- To visualise our findings, we could use a plot the proportion of groups of customers who bought 0, 1, or 2 coffees, and superimpose the theoretical binomial distribution on top:

```{r}
#| label: fig-coffee
#| fig-cap: "Proportion of couples who bought 0-2 coffees"

grid.x = seq(min(tips$HadCoffee), max(tips$HadCoffee), 1)
grid.y = dbinom(grid.x, 2, p_coffee)


coffee_plot <- ggplot() +
    geom_bar(data = tips, aes(x = coffee_est, y = after_stat(prop))) + 
    geom_point(data = tibble(grid.x, grid.y), aes(grid.x, grid.y), color = 'red') + 
    xlim(-1, 3) + 
    labs(title = "Proportion of couples who bought 0-2 coffees", x = "No. of Coffees Purchased")
coffee_plot 
```
