...
- What's the probability of a viewer giving a good rating after watching a Drama movie[^p-genre-good]?
...
- What's the probability of a viewer giving a good rating after watching an Action movie[^p-genre-good]?

- What's the probability of a viewer giving a good rating after watching an Comedy movie[^p-genre-good]?

- What’s the probability of a non-comedy movie being rated as Bad[^p-bad]?

[^p-bad]: Hint: If we were to ask whats the probability of a non-human being short, we could calculate the following:

    P(Droid and short) + P(Ewok and short) + P(Wookiee and short) = 0.10256410 + 0.02564103 + 0.00000000 = 0.1282051.
                
    Alternatively, we could do the "sum" of "short" minus the value of "human" and "short" i.e., 0.51282051 - 0.38461538 = 0.1282051.

    ```{r}
    swars_rel_freq_sum
    #P(non-human and short) = P(Droid and short) + P(Ewok and short) + P(Wookiee and short) = 0.10256410 + 0.02564103 + 0.00000000 ≈ 0.13
    ```

- What’s the probability of a movie rater watching a Drama movie **or** rating a movie as Bad[^p-dr-bad]?
...
- Do you think that rating is independent of Genre?[^genre-indep]
...
[^func-mosaic]: Hint: Make sure to load the `ggmosaic()` package so that you can specify `geom_mosaic()` when building your plot. To add a title, as well as x- and y-axis titles, specify `labs(title = , x = , y = )`. This week we will also need to specify the `conds()` argument. 
    ...
    m_plot <- ggplot(starwars2) +
        geom_mosaic(aes(x = product(species), fill = species, conds = product(size))) +
...
- What’s the probability of a customer tipping above 15%?
...
- Is tipping is independent of the server?

No, the events seem to be dependent. The conditional probabilities are different from P(above). In particular, the probability of tipping above average after service from Server A is higher.
...
- To visulaise our findings, we could use a conditional mosaic plot:
...
mos_cond_plot <- ggplot(tips2) +
    geom_mosaic(aes(x = product(Server), fill = Server, conds = product(Tip_Avg))) +  
    labs(title = "Conditional Association between Servers and Tips", x = "Tip Average", y = "Server", fill = "Server")
mos_cond_plot
```
