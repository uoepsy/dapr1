...
In the report write-up do not include the intermediate working that was necessary to compute the probabilities required, it's sufficient to define the probability and give the final result.

Following APA style, do not use a zero before a decimal when reporting probabilities.

The working should be part of the code, instead, which will appear in Appendix B.
:::
...
[^p-good]: Hint: For the starwars example data, if we were to ask what is the probability of a species being short, i.e. P(short), we would look at the relative frequencies table:
    ...
    0.10256410 + 0.02564103 + 0.38461538 + 0.00000000
    ```
    
    Alternatively, we would use look in the "Sum" value under the "short" column of our table. 

    > We can write this up as: the probability of a Starwars character being short was .51.
...
[^p-genre-good]: Hint: For the starwars example data, if we were to ask what is the probability of being short for the Human species, i.e. P(short | human), we would use the relative frequencies table 
    ...
    0.38461538 / 0.79487179
    ```
    
    > We can write this up as: given that a viewer watched a drama movie, the probability that they rated it as good was .48.
...
[^p-bad]: Hint: For the Starwars example data, if we were to ask what is the probability of a character being not a human given they are short, we would use the relative frequencies table
    ...
    ```{r}
    # P((~human) | short) = 
    #   ( P(droid AND short) + P(ewok AND short) 
    #       + P(wookiee AND short) ) / P(short)
    (0.10256410 + 0.02564103 + 0.00000000) / 0.51282051
    ```
    
    > We can write this up as: Given that a Starwars character is short, the probability that they are a non-human is .25.
...
$$
\begin{aligned}
P\big((\sim A_3) \mid B\big)
&= P\big( (A_1 \cup A_2) \mid B \big) \\
...
- Given that a customer tipped below average, what's the probability of them being served by B or C?
...
> Given that a customer tipped below average, the probability of the customer being served by either server B or C was $P((Server~B \cup Server~C) \mid Below) = .65$
