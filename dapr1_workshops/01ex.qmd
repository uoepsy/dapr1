...
You can access the RStudio server by following the instructions on the [FAQs page](https://uoepsy.github.io/dapr1/2627/misc/faqs-dapr1.html){target="_blank"} (see first drop down box).  
...
To get out of this we can either type the required stuff to finish the query, or, much more simply, just press the Escape key (Esc) on your keyboard and it will cancel the query and let you start it again.  
...
3. You'll notice that writing in the script doesn't actually perform the calculations. We need to "Run" the commands we are writing, which we can do by ensuring the cursor is on the line we want to run, and then either:  
    - press Ctrl+Enter (or Cmd+Enter for Mac)<br>OR<br>
...
#| eval: false
#| echo: true
# starting a line with a "#" turns it into a comment
# it means that R will not try to run it  

# we can store things in R's memory using "name <- value", e.g.:
my_name <- "Umberto"
about_me <- "Italian."
fun_fact <- "I can play the piano."

# this -- paste() -- is a "function". 
# it takes some inputs, and gives us outputs
# in this case, we give it a whole load of separate sentence fragments
...
1. Copy-Paste the lines of code above into your R Script and edit them to provide some text about yourself (if you don't want to share a fun fact about yourself, just share a random fun fact!)
...
Groups are named based on the workshop (1, 2, 3, 4, 5) and your table number.  
So if this is Workshop 3, and you are sitting at table 6, you should join group "Group 3_06".  

1. Go to the course LEARN page
2. click Groups on the top menu bar.
3. click Workshops_1_2_3_4_5
4. Join your group
5. Now click on "Group Discussion Space" within the Quick Links folder of LEARN.
6. Upload your script to the discussion space, to share your fun fact with your group members.  
    - If you want step-by-step instructions for how to get your file from the RStudio server to post on the discussion space, then click [here](00serverfiles.html){target="_blank"}.    
