---
title: "W5: Summarising Distributions II – Variability"
params: 
    # SHOW_SOLS: TRUE
    SHOW_SOLS: FALSE
    TOGGLE: TRUE
...
We've provided below the range of all 3 variables, so you can check your answer whichever one you chose.

```{r}
sdata |> 
  summarise(
    sleep_R = max(sleepqual, na.rm = TRUE) - min(sleepqual, na.rm = TRUE),
    procr_R = max(procrast, na.rm = TRUE) - min(procrast, na.rm = TRUE),
    multi_R = max(multitask, na.rm = TRUE) - min(multitask, na.rm = TRUE)
  )
```
...
For chronotype --- as measured by a sliding scale from 0 ("evening person") to 10 ("morning person") --- the distribution was **(6) positively skewed **. Consequently, the central tendency for chronotype is best described by the **(7) median** of **(8) `r median(sdata$ampm)` ** (**(9) IQR = `r IQR(sdata$ampm)` **), with scores along the entire range from **`r min(sdata$ampm)`** to **`r max(sdata$ampm)`**.
