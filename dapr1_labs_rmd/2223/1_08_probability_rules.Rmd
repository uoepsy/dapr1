...
Be sure to check the [**solutions to last week's exercises**](1_07_probability_basics.html).  
...
`r optbegin("In pictures: Independence vs Dependence", olabel = FALSE, show = params$SHOW_SOLS, toggle = params$TOGGLE)`
...
as factors (`<fct>`). Furthermore, we could give better names to the Tip column. The function `factor` lets you specify the values of a variable, and the labels those values should be mapped to when creating a factor variable: 

```{r}
tipjoke <- tipjoke %>%
  mutate(
    Card = factor(Card),
    Tip = factor(Tip, levels = c(0, 1), labels = c('Not tipped', 'Tipped'))
  )
...
- $T_1^- = \text{individual 1 tests negative}$
- $T_2^- = \text{individual 2 tests negative}$
- $C_1^+ = \text{individual 1 has COVID-19}$
- $C_2^+ = \text{individual 2 has COVID-19}$.

Calculate $P(T_1^- \cap T_2^- \mid C_1^+ \cap C_2^+)$.

Once you expand the calculation above, you can make the following simplifications:
...
We wish to find $P(T_1^- \cap T_2^- \mid C_1^+ \cap C_2^+)$, i.e. the probability that both individuals will test negative given that both have COVID-19.  

The two events that "individual 1 tests negative" and "individual 2 tests negative" are both independent given the information that "both have COVID-19".
This is because the probability that one individual tests negative, given that both have COVID-19, is not affected by the other individual's test.

For two independent events $A$ and $B$, we saw that:

$$
P(A \cap B) = P(A) \times P(B)
$$

Similarly, two events $A$ and $B$ are conditionally independent given $U$ if:

$$
P(A \cap B | U) = P(A | U) \times P(B | U)
$$

In our case, A = "individual 1 tests negative" = $T_1^-$, B = "individual 2 tests negative" = $T_2^-$, and U = "both have COVID-19" = $C_1^+ \cap C_2^+$. This leads to:

$$
P(T_1^- \cap T_2^- \mid C_1^+ \cap C_2^+) = P(T_1^- \mid C_1^+ \cap C_2^+) \times P(T_2^- \mid C_1^+ \cap C_2^+)
$$

We are also told that:

- $P(T_1^- \mid C_1^+ \cap C_2^+) = P(T_1^- \mid C_1^+)$
- $P(T_2^- \mid C_1^+ \cap C_2^+) = P(T_2^- \mid C_2^+)$ 

And the formula can be simplified to:

$$
P(T_1^- \cap T_2^- \mid C_1^+ \cap C_2^+) = P(T_1^- \mid C_1^+) \times P(T_2^- \mid C_2^+)
$$

$P(T_1^- \mid C_1^+)$ and $P(T_2^- \mid C_2^+)$ are both the probability of an individual testing negative, given that they have COVID-19, which represents a feature of the test device and will be the same for any individual $i$. We already found this probability in question 5: $P(T^- | C^+) = 0.029$. 

The probability of both individuals testing negative is then: 

$$
\begin{aligned}
P(T_1^- \cap T_2^- \mid C_1^+ \cap C_2^+) 
&= P(T_1^- \mid C_1^+ \cap C_2^+) \times P(T_2^- \mid C_1^+ \cap C_2^+) \\
&= P(T_1^- \mid C_1^+) \times P(T_2^- \mid C_2^+) \\
&= 0.029 \times 0.029  \\
&= 0.000841 \\
&\approx 0.08 \%
\end{aligned}
...
a. Suppose 3\% of the population of Scotland currently has COVID-19. What is the probability that a randomly selected person (from Scotland) has the disease _and_ tests positive? 

b. Suppose instead that 30\% of the population of Scotland currently has COVID-19. What is the probability that a randomly selected person (from Scotland) has the disease _and_ tests positive? 

c. Based on those numbers, what can you say about the probability of observing a true positive in the population?

`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
a. The question tells us that $P(C^+) = 0.03$.
...
b. The question tells us that $P(C^+) = 0.3$.
...
c. If you compare your answers to (a) and (b) you can see that, for a test with a given accuracy, the probability of observing a true positive depends on how prevalent the disease is in the population, i.e. how many people have COVID-19 in the population.
