...
---
# Write up
An independent sample $t$-test was used to assess whether the maths score mean of the control group $(n = `r calc[2,4]`)$ was higher than that of the stereotype threat group $(n = `r calc[1,4]`)$. There was a significant difference in test score between the control $(Mean=`r calc[2,2]`; SD=`r calc[2,3]`)$ and threat $(Mean=`r calc[1,2]`; SD=`r calc[1,3]`)$ groups, where the scores were significantly higher in the control group ( $t$(`r res$parameter`)=`r round(res$statistic,2)`, $p< .05, one-tailed)$. Therefore, we can reject the null hypothesis. The direction of difference supports our directional hypothesis and indicates that the threat group performed more poorly than the control group.
