...
plt1 <- ggplot(tibble(x = xbar.sampl.dist)) + 
    ...
    stat_function(aes(x = x), fun = \(x) dnorm(x, avg.xbar, se.xbar),
                  color = '#d95f02', size = 1, linetype = 1) +
...
plt.t <- ggplot(tibble(t = tscores.sampl.dist)) + 
    geom_histogram(aes(x = t, y = ..density..), 
                   ...
                   color = 'gray50', fill = 'gray90') +
    stat_function(aes(x = t), fun = dnorm, color = '#d95f02',
                  size = 1, linetype = 1, n = 501) +
    stat_function(aes(x = t), fun = \(x) dt(x, df = n-1),
                  color = '#7570b3', size = 1, n = 501) +
    labs(x = 't-statistic', y = 'Density',
         caption = 'Orange: Standard Normal\nPurple: t(n-1)') +
...
ggplot(plt, aes(x, y)) +
    ...
    geom_area(data = plt.pval, aes(x = x, y = y),
              fill = '#1b9e77', alpha = 0.75) +
    geom_vline(xintercept = tobs, color = '#1b9e77', size = 1) +
    annotate('text', tobs + 0.3, 0.03, 
             label = expression(t[obs]),
             color = '#1b9e77', size = 7, adj = 0) +
    labs(x = 't-statistic', y = 'Density',
...
- In doing so, we compared the _green_ area, corresponding to the p-value, against the _orange_ area, corresponding to the $\alpha = 0.05$ significance level.
...
ggplot(plt) +
    geom_line(aes(x, y), color = '#7570b3', linewidth = 1) +
    geom_area(data = plt.crit,
              aes(x = x, y = y), fill = '#d95f02', alpha = 0.75) +
    geom_area(data = plt.pval,
              aes(x = x, y = y), fill = '#1b9e77', alpha = 0.75) +
    annotate('text', x = tcrit[2], y = 0.2,
             label = expression(alpha == 0.05),
             color = '#d95f02', adj = 0, size = 5) +
    annotate('text', x = tobs, y = 0.1,
             label = paste('p =', signif(pvalue, 2)),
             color = '#1b9e77', adj = 0, size = 5) +
    labs(x = 't-statistic', y = 'Density')
```
]
...
.pull-left[
- Rather than comparing the area of $\alpha$ (0.05, in orange) to the area of the p-value (0.018, in green), we can compare the corresponding t-statistics along the x-axis.
...
ggplot(plt) +
    geom_line(aes(x, y), color = '#7570b3', linewidth = 1) +
    geom_vline(xintercept = tcrit, color = '#d95f02', size = 1) + 
    geom_vline(xintercept = tobs, color = '#1b9e77', size = 1) +
    geom_area(data = plt.crit,
              aes(x = x, y = y), fill = '#d95f02', alpha = 0.75) +
    geom_area(data = plt.pval,
...
ggplot(plt) +
    ...
    geom_area(data = plt.crit,
              aes(x = x, y = y), fill = '#d95f02', alpha = 0.75) +
    geom_area(data = plt.pval,
              aes(x = x, y = y), fill = '#1b9e77', alpha = 0.5) +
    annotate('label', x = c(-5, 5), y = c(0.3, 0.3), 
             label = c(
                 expression(frac(alpha, 2) == 0.025), 
                 expression(frac(alpha, 2) == 0.025)
...
mosaic::xqt(0.05, df = 3, 
            return = 'plot') +
    scale_fill_manual(
        values = c('#d95f02', 'gray90')
    ) +
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu < mu[0])) +
    geom_vline(xintercept = tcrit, col = '#d95f02', size = 1) +
    annotate('label', x = -5, y = 0.2, colour = '#d95f02', size = 6,
             label = expression(alpha == 0.05)) +
    annotate('text', x = tcrit - 0.2, y = -0.02, hjust = 1,
             colour = '#d95f02', size = 6,
...
mosaic::xqt(0.05, df = 3, 
    ...
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu < mu[0])) +
    geom_vline(xintercept = c(-5.2, -1.8, 1.5), colour = '#1b9e77',
               size = 1) +
    annotate('label', x = c(-5.2, -1.8, 1.5), y = c(0.1, 0.1, 0.1), 
             label = c('A', 'B', 'C'), size = 6, colour = '#1b9e77') +
    geom_vline(xintercept = tcrit, col = '#d95f02', size = 1) +
    annotate('label', x = -5, y = 0.2, colour = '#d95f02', size = 6,
             label = expression(alpha == 0.05)) +
    annotate('text', x = tcrit - 0.2, y = -0.02, hjust = 1,
...
mosaic::xqt(0.95, df = 3,
    ...
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu > mu[0])) +
    geom_vline(xintercept = c(-1.5, 1.8, 5.2), colour = '#1b9e77',
               size = 1) +
...
mosaic::xqt(c(0.025, 0.975), df = 3,
    ...
    labs(x = 't-statistic', y = 'Density',
         title = expression(H[1] : mu != mu[0])) +
    geom_vline(xintercept = c(-5.3, -2, 1.3, 3.7), colour = '#1b9e77',
               size = 1) +
    annotate('label', x = c(-5.3, -2, 1.3, 3.7), y = c(0.1, 0.1, 0.1, 0.1), 
             label = c('A', 'B', 'C', 'D'), size = 6, colour = '#1b9e77') +
    geom_vline(xintercept = tcrit, col = '#d95f02', size = 1) +
    annotate('label', x = -5, y = 0.2, colour = '#d95f02', size = 6,
             label = expression(frac(alpha,2) == 0.025)) +
    annotate('label', x = 5, y = 0.2, colour = '#d95f02', size = 6,
