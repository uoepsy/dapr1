...
- The goal is to use the sample data to answer questions about the population data. For example:
    ...
    + What's the population mean? Goal: estimating a parameter
    
    + Could the population mean be zero? Goal: testing hypotheses about a parameter.
...
- In one picture we can see the dog, but we would like to know where the lady is.
]
...
- We have one picture, i.e. one figure, which corresponds to one sample.
...
- $t = \frac{47 - 50}{1.004} = 2.99$

- The sample mean (47) is about 3 standard errors from the claimed mean (50)
]
...
mtext(side=1, text = xvals, at = xvals, adj = 0.5, padj = 1.3, col = 'blue')
mtext(side=1, text = tvals, at = xvals, adj = 0.5, padj = 4, col = 'darkorange')
abline(v = xbar, col = 'darkgreen', lwd = 2)
text(53, 0.38, label = expression(bar(x) * ' ' * values), col = 'blue', 
     cex = 1.1, adj = 0)
text(53, 0.35, label = expression(t * ' ' * values), col = 'darkorange', 
     cex = 1.1, adj = 0)
text(xbar, 0.10, label = expression(bar(x) == 47), col = 'darkgreen', 
     cex = 1.1, adj = 1.18)
text(xbar, 0.07, label = expression(t == -2.99), col = 'darkgreen', 
     cex = 1.1, adj = 1.1)

arrows(50, 0.1, 50, 0, col = 'purple')
...
```{r, echo=T}
pvalue <- pt(-abs(tobs), df = n - 1) + 
          pt( abs(tobs), df = n - 1, lower.tail = FALSE)
pvalue
```

- We could say:

> At the 5% significance level, the sample data provide very strong evidence to reject the null hypothesis in favour of the alternative one that the mean body temperature of healthy humans is different from the long thought 37 °C; $t(49) = -3.14, p = 0.003$, two-sided.
...
- Notice that the 95% confidence interval [36.69, 36.93] does not include the hypothesised value in the null hypothesis, $\mu_0 = 37$.
...
- As 37 is not within the range, that is not a plausible value for $\mu$ and if you test whether the population mean could be equal to 37 you end up rejecting that value.

--

- The 95% CI gives you a range of values for the population mean that would not be rejected at the 5% significance level in a two-sided hypothesis test.
...
- The p-value is 0.003, indicating that the sample data provide very strong evidence that the population mean is significantly different from 37 °C.
...
- You can have a significant result, but this doesn't necessarily mean that it's __important__. 
    ...
    + It's good practice to always follow-up a significant result with a confidence interval, to provide the reader with a measure of the "magnitude" of the departure from the hypothesised value $H_0 : \mu = \mu_0$.
...
- The answer is that they both give you useful and complementary information.
...
- In summary, report both if you have a significant result!
