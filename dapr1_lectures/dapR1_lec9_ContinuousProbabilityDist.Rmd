...
3. Understand how to use a range from a continuous probability distributions
...
- Continuous probability distributions differ from discrete in two other important ways
  ...
  - Continuous probability distributions are described using the **probability density function (PDF)**, rather than the **probability mass function** (We'll come back to this point in a second...)
...
- But the basic point is that it is a function of:
...
- With a discrete random variable it only makes sense to ask: 'What's the probability associated with a specific value of the random variable?'.
...
--
  ...
  - Why?  The total probability (1) is divided between an infinite number of possible values that the variable could take, as the variable is continuous
...
+ First, let's draw these boundaries on our plot
