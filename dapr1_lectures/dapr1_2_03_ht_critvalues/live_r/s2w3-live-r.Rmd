...
- Did you install `tinytex`? If not,
...
- Ensure the metadata are correctly written:
...
- Ensure there are no unicode symbols copied and pasted in your file.
    ...
    For example, $\alpha$ should be written within an equation as `$\alpha$`. All those symbols should be written within equations blocks (delimited by dollars).
    
    Another example is $\cup$ for $P(A \cup B)$. This should be written as `$P(A \cup B)$`
    
    Another example is $H_{0}$, which should be written `$H_{0}$`
