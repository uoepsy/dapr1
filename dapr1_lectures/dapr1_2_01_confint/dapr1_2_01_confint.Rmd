...
- $Z \sim N(0,1)$ is a standardised normal distribution
...
- Recall that for a random variable $X$ which follows a $N(\mu, \sigma)$ distribution, 95% of the values are between -2 and 2:

```{r}
knitr::include_graphics('images/normal_rule.png')
```
...
$$
\begin{aligned}
z = -1.96 &\rightarrow x = \mu - 1.96 \cdot \sigma \\
z = 1.96 &\rightarrow x = \mu + 1.96 \cdot \sigma
\end{aligned}
$$
...
```{r}
knitr::include_graphics('images/bias_prec.png')
```
...
```{r}
knitr::include_graphics('images/2_01_sampl_dist.png')
```
...
```{r}
knitr::include_graphics('images/2_01_sampl_dist_t.png')
```
...
```{r}
knitr::include_graphics('images/2_01_sampl_dist_t_2.png')
```
...
```{r}
knitr::include_graphics('images/2_01_one_sample.png')
```
...
- If we do NOT have the population data, we cannot compute $\mu$ but also $\sigma$, the population standard deviation.

- Recall that $\sigma$ is required to compute the precision of the sample mean:

$$
SE = \frac{\sigma}{\sqrt n}
$$

- How can we compute the SE of the mean if we can only afford one sample comprising $n$ individuals?
...
- Range of plausible values for the parameter, called __confidence interval__. More likely that the true value will be within a range.
...
- Confidence level = a number between 0 and 1 specified by us. How confident do we want to be that the confidence interval will contain the true parameter value?
...
- Recall, however, that 
  ...
  + we also don't have $\sigma$ so we need to estimate it with $s$, the sample standard deviation:
...
- The quantiles -1.96 and 1.96 refer to the normal distribution, so these are wrong!
...
---
# Example: 95% CI for the population mean salary
