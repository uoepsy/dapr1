...
A $\chi^2$ Goodness of Fit test was conducted in order to determine whether the distribution of flowers ordered across each day of the week was equivalent. The goodness of fit test was significant $(\chi^2(df = 6, n= 7) = 16.62, p = .011)$, and thus the null hypothesis was rejected as the distribution of flower orders differed across the days of the week. 
...
+ The probability that we would have a $\chi^2$ value as extreme as `r round(sum(divTab[flowerCols]), 3)` if $H_0$ was true is `r round(pchisq(sum(divTab[flowerCols]),df = 6,lower.tail = F), 3)`.
...
A $\chi^2$ test of independence was performed to examine whether the type of flower sold was independent of season. There was no significant association between these variables $(\chi^2(6) =  2.40, p = .880)$. Therefore, we failed to reject the null hypothesis.
...
+ Cramer's V is interpreted based on $df^*$, where Cramer's V must lie between 0 and 1 (0 = complete independence, 1 = complete dependence):
