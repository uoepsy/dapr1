...
- Similarly, there are two types of errors in hypothesis testing.
  ...
  You could reject "not guilty" when the person is truly "not guilty"  
  You could reject the null hypothesis when it is true
  
  - You could fail to convict a guilty defendant  
  You could fail to reject "not guilty" when the defendant is guilty  
  You could fail to reject the null hypothesis when it is false

--

- Like convicting an innocent person, the error of rejecting a true null hypothesis is considered serious, and so a null hypothesis isn't rejected unless the evidence against it is convincing beyond reasonable doubt.
...
- $\alpha = 0.05$ (in red) = P(Rejecting $H_0$ | $H_0$ is true)
...
- Alternative states that $\mu = 1.5$
...
- Effect of increasing the sample size. Remember: the sampling distributions will get narrower as $SE = \frac{\sigma}{\sqrt n}$
...
- Effect of increasing the population SD $\sigma$.

- Sampling distribution will have a larger spread as $SE = \frac{\sigma}{\sqrt n}$
...
- Effect of decreasing the population SD $\sigma$.

- Sampling distribution will have a lower spread as $SE = \frac{\sigma}{\sqrt n}$
...
- We saw that a statistically significant result may not be important at all, i.e. may not have much real-world value.
    ...
    + Importance is related to the practical distance between the hypothesised value  and the true population mean. That is, it is related to the effect size.
...
- Dividing this by the SD of the data (which is in the same unit of measurement as the data itself), gives you a measure that does not depend on the unit of measurement.
...
x <- rchisq(10000, 4)
hist(x, main = "Population data", 
     col = 'lightblue', border = 'lightblue4', breaks = 30,
     xlab = "Data", cex.main = 1.5, cex.lab = 1.5)
