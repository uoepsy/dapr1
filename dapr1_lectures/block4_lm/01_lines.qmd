...
junk::course_table(block1_name,block2_name,block1_lecs,block2_lecs,week=11)
```
...
junk::course_table(block3_name,block4_name,block3_lecs,block4_lecs,week=6)
```
...
p3_1 <- p3 + 
  ...
  geom_abline(intercept=coef(m3)[1],slope=coef(m3)[2],
              col="#0F4C81",lwd=1) +
  geom_point(x=0,y=coef(m3)[1],size=4,col="#0000FF")+
...
p3a_1 <- p3a + 
  ...
  geom_abline(intercept=coef(m3a)[1],slope=coef(m3a)[2],
              col="#0F4C81",lwd=1) +
  geom_point(x=0,y=coef(m3a)[1],size=4,col="#0000FF")+
...
> **RQ:** How does an additional night of interrupted sleep affect cognitive skills?  <br><br>
> We recruit 80 people and randomly allocate them to receiving between 0 to 7 days of sleep disruption. Upon waking up after their final night of allocated disrupted sleep, participants completed a short task based on Wordle, that gives a score somewhere between 0 and 100 (lower scores are better)
...
#| echo: true
#| eval: false
cor.test(sleepwordle$n_disrupted, 
         sleepwordle$wordle)
```
:::{style="font-size:.7em"}
```{r}
#| echo: false
cor.test(sleepwordle$n_disrupted, sleepwordle$wordle)
```
:::
...
$$
\begin{align}
\color{red}{\text{Wordle_Score}} &= \,\,\,\color{blue}{b_0 \,\,\,+ b_1 \cdot \text{Nights_Disrupted}} + \color{black}{\varepsilon} \\
\end{align}
$$
...
$$
\begin{align}
\color{red}{\text{Wordle_Score}} &= \,\,\,\color{blue}{245 \,\,\,+ 37 \cdot \text{Nights_Disrupted}} + \color{black}{\varepsilon}
\end{align}
$$
...
$$
\begin{align}
\color{red}{\text{Wordle_Score}_i} &= \,\,\,\color{blue}{b_0 \,\,\,+ b_1 \cdot \text{Nights_Disrupted}_i} + \color{black}{\epsilon_i} \\
\end{align}
$$
...
$$
\begin{align}
\color{red}{\text{Wordle_Score}_i} &= \,\,\,\color{blue}{245 \,\,\,+ 37 \cdot \text{Nights_Disrupted}_i} + \color{black}{\epsilon_i}
\end{align}
$$
...
$$
\begin{align}
\color{purple}{\widehat{\text{Wordle_Score}}_i} &= \color{blue}{245 + 37 \cdot \text{Nights_Disrupted}_i}  
\end{align}
$$
:::
...
sleepwordle |> 
  ggplot(aes(x=n_disrupted,y=wordle))+
  geom_point(size=3,alpha=.3,col="tomato1")+
  stat_smooth(method=lm,se=FALSE,color="blue",lwd=2)
```
...
sleepwordle |> 
  ...
  geom_point(size=3,alpha=.3,col="tomato1")+
  stat_smooth(method=lm,se=FALSE,color="blue",lwd=2)+
  geom_segment(x=5,xend=5,y=sleepwordle$wordle[61],
               yend = (coef(fitw) %*% c(1,5))[1],
               lty = "dotted", col="black", lwd=1) +
  geom_point(x = 5, y = sleepwordle$wordle[61],
             size = 3,col="tomato1") +
  geom_point(x = 5, y = (coef(fitw) %*% c(1,5))[1],
             size = 3,col="purple")
...
:::{.fragment fragment-index=2 style="font-size:.7em"}
For this poor participant who had 5 nights of disrupted sleep, we predict their score to be:  
$$
\begin{align}
\color{purple}{\widehat{\text{Wordle_Score}}_i} &= \color{blue}{245 + 37 \cdot \text{Nights_Disrupted}_i} \\
&= \color{blue}{245 + 37 \cdot 5} \\
&= \color{blue}{245 + 185} \\
&= \color{blue}{430} \\
\end{align}
$$
...
1. predictions x0,x5,x7

TODO line of predictions
...
what happens to someone's wordle score when i disrupt their sleep for another night?

predict(person_i, days = person_i days) vs 
  predict(person_i, days = person_i days + 1) 

oh look, it's that coefficient! 
...
Read as `outcome` "is predicted by" `1` intercept plus a slope across the `predictor`
...
## The How-to - predictions {.smaller}

1. make a new little dataframe for values of $x$:
```{r}
data.frame(n_disrupted = 0:7)
```

2. get predictions of $y$ for each value of $x$:
```{r}
predict(sleep_mdl, 
        newdata = data.frame(n_disrupted = 0:7))
```


<!-- 3. get predictions for observed data: -->
<!-- ```{r} -->
<!-- predict(sleep_mdl) -->
<!-- ``` -->
