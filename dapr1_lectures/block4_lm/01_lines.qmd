...
but a linear model can be **used** - 
  ...
  - used how? to make predictions and to estimate a quantity of interest! 

	
## Example

> **RQ:** How does an additional day of interrupted sleep affect cognitive skills?  
> We recruit 80 people and randomly allocate them to receiving between 0 to 7 days of sleep disruption. <br>
...
```{r}
#| echo: true
cor.test(sleepwordle$sleep_disrupt, sleepwordle$wordle)
```
...
\quad \\
\color{red}{\text{Wordle_Score}} &= \,\,\,\color{blue}{b_0 \,\,\,+ b_1 \cdot \text{Days_Disrupted}} + \color{black}{\varepsilon} \\
\quad \\
\color{red}{\text{Wordle_Score}} &= \,\,\,\color{blue}{245 \,\,\,+ 37 \cdot \text{Days_Disrupted}} + \color{black}{\varepsilon}
\end{align}
$$
...
```{r}
#| echo: false
fitw = lm(wordle~sleep_disrupt,sleepwordle)
...
sleepwordle |> 
  ggplot(aes(x=sleep_disrupt,y=wordle))+
...
\quad \\
\quad \\
\color{red}{\text{Wordle_Score}_i} &= \color{blue}{b_0 + b_1 \cdot \text{Days_Disrupted}_i} + \color{black}{\epsilon_i}
\quad \\
\quad \\
\color{purple}{\widehat{\text{Wordle_Score}}_i} &= \color{blue}{b_0 + b_1 \cdot \text{Days_Disrupted}_i}  
\end{align}
$$
:::
:::{.column width="50%"}
