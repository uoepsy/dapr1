...
## Purpose
- The independent samples $t$-test is used when we want to test whether the mean of one group is statistically different from/greater than/less than the mean of another group

<br>

- Distinct/independent samples of participants:
    - Is the average reaction time different between participants who do and don’t drink caffeinated drinks?
...
- Independent samples $t$-test (if you **do** have equality of variances ^[if you do not have equality of variances, need to calculate via `cohens_d()` function from `effectsize` package - do not calculate by hand]):
