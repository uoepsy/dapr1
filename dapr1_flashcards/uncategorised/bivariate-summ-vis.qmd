---
title: "Summarise and visualise relationships between variables"
---
...
- ggmosaic doesn't exist for EP's version of R, add installation instructions in case students have same issue
- "Helpful references" section somewhat redundant, streamline
...
```{r echo=FALSE}
set.seed(421)
sample_n(stroopdata, n()) |> select(practice,stroop_effect) |> mutate(stroop_effect = round(stroop_effect,2)) |> head(9L) |> rbind("...")
```
...
p2<-ggplot(df,aes(x=x,y=y))+
  ...
  annotate("text",x=mean(df$x)+1, y=max(df$y)-5,label=TeX("$\\bar{x}$", output = "character"), parse = TRUE)+
  geom_hline(aes(yintercept=mean(y)), lty="dashed")+
  annotate("text",x=min(df$x)+5, y=mean(df$y)+1,label=TeX("$\\bar{y}$", output = "character"), parse = TRUE)
...
p3<-ggplot(df,aes(x=x,y=y))+
  ...
  geom_hline(aes(yintercept=mean(y)), lty="dashed")+
  annotate("text",x=min(df$x)+5, y=mean(df$y)+1,label=TeX("$\\bar{y}$", output = "character"), parse = TRUE)+
  annotate("text",x=mean(df$x)+7, y=202,label=TeX("$x_{i} - \\bar{x}$", output = "character"), parse = TRUE)+
  annotate("text",x=71.5, y=mean(df$y)+7,label=TeX("$y_{i} - \\bar{y}$", output = "character"), parse = TRUE)+
