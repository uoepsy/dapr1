...
::: {.callout-tip}
While we go through these settings, keep the ggplot2 package cheatsheet handy: <https://rstudio.github.io/cheatsheets/data-visualization.pdf>
:::
...
1. `ggplot(data, aes(...))`: specifies the data and which variables to plot
...
- use `fill` for inside areas (bars in barplots or histograms,  areas under density plots, interior of boxplots)

If you mix these up, your plot may not change in the way you expect.

::: {.callout-tip}
Bookmark the following page, as it lists the name of all colours in R: <https://r-charts.com/colors>
:::
...
- **Inside `aes()`**, e.g. `aes(fill = group)`: this maps a variable in your data to colour, so each group automatically gets a different
...
The option `fill` changes the colour of the bars (i.e. an area). In this case, it is set to cornflower blue.
...
The `colour` option, instead, sets the line colour to black. In this case, the line is just the border of the bars.
...
The `linewidth` option controls the thickness of a line, and it is set here to 1. The higher the value, the thicker the line will be.
...
We can also tell ggplot to change the colour or fill depending on the values within a variable in the data.
...
If you don't like the default colours, you can change them using a different colour palette.
The `scale_fill_brewer()` function allows you to choose from a range of colour palettes - see here for all the available ones: <https://ggplot2-book.org/scales-colour.html#brewer-scales>
