...
In general, we denote the two quantiles of the t-distribution as $-t^*$ and $+t_*$, so the interval is written in general as:
...
When we consider a single interval that we construct, it either includes the population mean or it doesn't. We just don't know as we don't have the population mean.
:::
