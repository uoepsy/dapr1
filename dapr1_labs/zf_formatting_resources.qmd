...
- [ ] **`%` signs in table captions**. A `%` sign in table captions can cause problems when knitting. If you want a `%` sign to appear in a table caption of the knitted report, write it as `\\%` within the table caption string in the Rmd file.

#### Final Notes {.unnumbered}

- [ ] **Tables formatted with `kbl()`**. Ensure that any tables appearing in the analysis section or appendix A are formatted as kables, i.e. passed to `kbl()` from the kableExtra package.
