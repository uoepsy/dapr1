...
- _critical value approach_: Compare the observed $t$-statistic with the appropriate 5% critical value(s) from a $t$-distribution. 
...
```{r}
tstar <- qt(c(0.025, 0.975), df = n_f + n_m - 2)
tstar
```


_Side note_: As you can see, the t-distribution is symmetric. The value that cuts an area of 0.025 to its left, $-t^* = -1.961585$ is equal to minus the value that cuts an area of 0.025 to its right, $+t^* = 1.961585$
$$
-t^* = -( +t^* )
$$
...
The observed t-statistic `r round(t_obs, 2)` is larger than the upper critical value `r round(tstar[2], 2)`. Hence, at the 5% significance level we have sufficient evidence against the null hypothesis that males and females have the same number of close friends.
...
ggplot(plot_df, aes(x = plot_x, y = plot_y))+
    ...
    geom_vline(xintercept = tstar, color = 'red') +
...
The sample results are significant, and we can also compute the effect size to check whether or not they are important.

```{r}
library(effsize)
cohen.d(num_close_friends ~ sex, data = gss, mu = 0, pooled = TRUE)
```

or

```{r}
cohen.d(gss$num_close_friends, gss$sex, mu = 0, pooled = TRUE)
```

As you can see above, the effect size, while being significant, is deemed to be of negligible importance.
...
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
First, we must find the appropriate critical values $\pm t^*$ (for the desired confidence level) from a t-distribution with degrees of freedom given by Welch's method (see Question 6 for the value).

The upper critical value for a 95% confidence level is:
```{r}
tstar <- qt(c(0.025, 0.975), df)
tstar
```

We compute a 95% confidence interval for the difference in population means, $\mu_{name} - \mu_{no\ name}$, as follows:
$$
(\bar x_{name} - \bar x_{no\ name}) \pm t^* \  \sqrt{\frac{s_{name}^2}{n_{name}} + \frac{s_{no\ name}^2}{n_{no\ name}}} \\
...
Including their name as part of the greeting to customers increases the server's tips, on average, by \$`r ci[1] %>% round(2)` to \$`r ci[2] %>% round(2)` per party.
