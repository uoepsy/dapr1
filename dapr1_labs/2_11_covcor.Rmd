...
Solutions are available, but the output of the R code is hidden. We invite you to type the code into R to investigate and get familiar with the output.
:::
...
```{r message=FALSE,warning=FALSE, results = params$CHUNK_RES, fig.show = params$CHUNK_RES}
library(tidyverse)
...
`r qend()`
`r solbegin(show = params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r, results = params$CHUNK_RES, fig.show = params$CHUNK_RES}
recalldata <-
  recalldata %>% mutate(
...
iii. Read in the job satisfaction data from [https://uoepsy.github.io/data/jobsat.csv](https://uoepsy.github.io/data/jobsat.csv). 

:::red
Use the function __read.csv()__ rather than __read_csv__ due to a problem with the package `polycor`. 
:::
...
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
```{r message=FALSE, results = params$CHUNK_RES, fig.show = params$CHUNK_RES}
