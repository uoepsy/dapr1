...
[Download the template Rmd file and start your work there](https://uoepsy.github.io/dapr1/2526/useful_files/template_formative_report_b.Rmd). Save your work regularly by clicking File > Save.

::: {.callout-important collapse="true"}
### Instructions - Don't Skip! {-}

Formative Report B covers the labs from weeks 7-11 of the DAPR1 course. You’ll need to create a PDF report using RMarkdown, which will be submitted by 12 noon on Friday, 28th November 2025. No extensions are available as this is a formative report. Expect written formative feedback in week 12 of semester 1.
...
Don't forget to register for your lab table group on LEARN by navigating to the course LEARN page, clicking on Groups, selecting Labs_1_2_3_4, finding your group, and clicking Join. 
...
|:-----------|:--------------|
| `filter` | Selects rows that meet a condition (tidyverse) |
| `mutate` | Adds or transforms columns in a data frame (tidyverse) |
| `drop_na` | Removes rows with missing values (tidyverse) |
| `factor` | Creates/converts to factors categorical variables with levels and labels (base R) |
| `ifelse(CONDITION, IF_TRUE, IF_FALSE)` | If CONDITION is true, use the value IF_TRUE, otherwise the value IF_FALSE (base R) |
| `table` | Makes contingency/frequency tables (base R) |
| `prop.table` | Converts counts to proportions (relative frequencies) from a table (base R) |
| `addmargins` | Adds row/column totals (margins) to a table (base R) |
| `all` | Tests whether all elements are jointly TRUE (base R) |
| `|` | OR operator: returns TRUE if at least one of the two arguments is TRUE |
| `geom_mosaic` | Draws mosaic plots for two categorical variables (ggmosaic, built on ggplot2) |
