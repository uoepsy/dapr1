...
- $\pm t^*$ are the appropriate quantiles of a t-distribution with $n - 1$ degrees of freedom
...
4. Outliers can be identified using boxplots. The `indentify_outliers()` function in the `rstatix` package can extract rows __suspected__ as outliers. 

# One sample t-test in disguise 

The paired-sample t-test is very similar to the one sample t-test covered in [Week 7](2_07_onesamplet.html).

__The steps for the one sample t-test are:__

1. calculate the sample mean ($\bar{x}$)

1. calculate the sample standard deviation $s$ of the data $x_i$

1. calculate the standard error $SE = \frac{s}{\sqrt{n}}$

1. calculate $t_{obs} = \frac{\text{estimate} - \text{null}}{SE} = \frac{\bar{x} - \mu_0}{SE}$

1. calculate the probability of observing a $t$-statistic at least as extreme, assuming the null hypothesis ($H_0: \mu = \mu_0$) is true.

<br>

__The steps for the paired t-test are:__

1. calculate the __difference score__ for each pair: $d_i = x_{i1} - x_{i2}$

1. calculate the sample mean of the differences: $\bar{d} = \sum d_i / n$

1. calculate the sample standard deviation of the differences, and denoted it $s_d$

1. calculate the standard error $SE = \frac{s_d}{\sqrt{n}}$

1. calculate $t_{obs} = \frac{\text{estimate} - \text{null}}{SE} = \frac{\bar{d} - \mu_0}{SE}$

1. calculate the probability of observing a $t$-statistic at least as extreme, assuming the null hypothesis ($H_0: \mu_d = \mu_0$) is true.


Typically, the hypothesised value $\mu_0$ is equal to 0. The formula for the t-statistic, assessing statistical significance, and constructing confidence intervals is identical to the one-sample t-test. 
...
$$
t_{obs} = \frac{\bar d - 0}{SE}, \qquad \text{where} \qquad SE = \frac{s_d}{\sqrt n}
$$
...
Equivalently, you could have computed it as:

```{r}
2 * pt(abs(t_stat), df = 24, lower.tail = FALSE)
```
...
# tstar
tstar <- qt(c(.025, .975), df = 24) 
tstar

ci <- aceterms$dbar + tstar * SE
ci
```
...
Or as a one-sample Cohen D but where the variable is __the difference__:

```{r}
library(effsize)
cohen.d(marriage_age$age_diff, NA)
cohen.d(marriage_age$Husband - marriage_age$Wife, NA)
```
...
At the point of recruitment, did the 60 participants smoke more or less than 20 cigarettes per day?
...
A paired-test was performed to assess if there was a significant difference in the average number of cigarettes smoked in participants who were in the experimental (app) condition. 
On average, compared to the initial timepoint ($M$ = 14.83), participants smoked less cigarettes at the one-month follow-up ($M$ = 9.70). 
This difference 5.13 [95% CI 3.01, 7.20], was statistically significant $t$(29) = 5.09, $p$ < .001, two-sided. 
