...
Although R is a programming language, it can also be used in interactive mode, and it is sometimes convenient to use it as a calculator. Once you have opened RStudio, the default view should show a panel in the bottom left, with a tab laballed "Console" (you may need to click on this tab to select it).  The console can be used to type single line commands. The commands are written after the '>' prompt, followed by pressing Return on your keyboard. In this way, you can use R as a calculator. For example, try typing the following at the console:

```{r}
2*8
```

The system should respond with the answer. Or, you could try something slightly more complex, for exampleL
```{r}
2*(8+3)
```
