...
#Because we are interested in running a hypothesis test on the correlation between `daysInProgram` and `cvHealth`, we can produce a scatterplot to check for linearity, outliers, and heteroskedasticity. Can also use this to visulaise the association between our two vairables:
...
We used Spearman's $\rho$ to determine whether there was a significant correlation between Difficulty and Satisfaction program ratings. We found a significant weak negative association between Difficulty and Satisfaction $(\rho = -.21, p < .001)$, such that those who found the program more difficult reported lower satisfaction with the program overall.
